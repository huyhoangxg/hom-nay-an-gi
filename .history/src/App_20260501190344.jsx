...
const SUPABASE_URL = 'https://abcxyz.supabase.cohttps://supabase.com/dashboard/project/gwdoaxroigzbjzfhztzb/settings/general'; 
const SUPABASE_ANON_KEY = 'gwdoaxroigzbjzfhztzb';
...
function App() {
  ...
  // GỌI DATA TỪ SUPABASE KHI LOAD TRANG
  useEffect(() => {
    fetchTestimonials();
  ...
  const fetchTestimonials = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('testimonials')
        .select('*')
        .order('created_at', { ascending: false }); // Lấy bài mới nhất lên đầu
      
      if (error) throw error;
      if (data) setTestimonials(data);
    } catch (error) {
      console.error('Lỗi khi lấy dữ liệu đánh giá:', error.message);
    } finally {
  ...
  const handleFeedbackSubmit = async (e) => {
    ...
    try {
      ...
      // Cập nhật state nội bộ để hiện ngay ra màn hình
      if (data) {
      ...
      alert('Đã có lỗi xảy ra khi gửi phản hồi. Vui lòng thử lại!');
    }
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-12">
            ...
            <div>
              ...
              <div className="space-y-3 text-gray-400 text-sm">
                <p>Email: support@homnayangi.vn</p><p>Điện thoại: 035 7188 616</p>
              </div>
            </div>
            <div>
