...
function App() {
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans bg-[#fdfaf5] selection:bg-brand selection:text-white" style={{ zoom: '90%' }}>
          ...
          <div className="relative h-[450px] md:h-[600px] rounded-[32px] overflow-hidden shadow-2xl bg-black">
             ...
             {isSpinning ? (
               <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-[#fff7ed] animate-fade-in">
                 ...
                 <p className="text-orange-600/80 mt-3 font-medium text-lg">Chuẩn bị chốt đơn nhé! 😋</p>
               </div>
             ) : randomFoodImage ? (
                /* 2. TRẠNG THÁI CÓ KẾT QUẢ RANDOM */
                <img 
                  src={randomFoodImage} 
                ...
                /* 3. TRẠNG THÁI BÌNH THƯỜNG (SLIDER) */
          ...
          <div className="max-w-4xl mx-auto rounded-[32px] overflow-hidden shadow-2xl shadow-orange-900/10 border-4 border-white bg-white group hover:shadow-orange-500/20 transition-all duration-500 hover:-translate-y-2">
            <div className="relative w-full" style={{ paddingBottom: '56.25%' }}>
              <iframe 
                ...
                src="https://youtu.be/rqNbAvQYZUg?si=ytTymI_zISsCbyEc"
              ...
              {testimonials.length === 0 ? (
               ...
               testimonials.map((item) => (
                <div 
                  ...
                  className="w-[320px] md:w-[400px] h-[420px] md:h-[480px] snap-center shrink-0 bg-white rounded-[32px] p-6 md:p-8 shadow-sm hover:shadow-md transition-shadow flex flex-col border border-gray-100 animate-fade-in"
                >
                  {/* Phần 1: Header (Avatar + Tên) - Giữ cố định trên cùng */}
                  ...
                  {/* Phần 2: Đánh giá Sao - Giữ cố định */}
                  <div className="shrink-0 mb-2">
                    <RenderStars rating={item.rating} />
                  </div>
                  
                  {/* Phần 3: Nội Dung + Ảnh - Tự động có thanh cuộn mượt mà nếu quá dài */}
                  <div className="flex-1 overflow-y-auto overflow-x-hidden pr-2 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:bg-gray-200 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-transparent">
                    <p className="text-textGray leading-relaxed italic break-words">"{item.content}"</p>
                    
                    {item.image_url && (
                      <div className="mt-4 rounded-2xl overflow-hidden border border-gray-100 bg-gray-50 flex justify-center shrink-0">
                        <img src={item.image_url} alt="Review attachment" className="w-full h-40 sm:h-48 object-contain hover:scale-105 transition-transform duration-500" />
                      </div>
                    )}
                  </div>
                </div>
      ...
      {/* Khung Popup Video (Sẽ chỉ hiện khi isVideoModalOpen = true) */}
      {isVideoModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
          <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => setIsVideoModalOpen(false)}></div>
          <div className="relative w-full max-w-4xl bg-black rounded-2xl overflow-hidden shadow-2xl border-4 border-white/10 z-10 animate-fade-in">
            <button onClick={() => setIsVideoModalOpen(false)} className="absolute -top-4 -right-4 sm:top-4 sm:right-4 z-20 bg-red-500 text-white rounded-full p-2 hover:bg-red-600 transition outline-none">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
            </button>
            <div className="relative w-full" style={{ paddingBottom: '56.25%' }}>
              <iframe className="absolute top-0 left-0 w-full h-full" src="https://youtu.be/rqNbAvQYZUg?si=ytTymI_zISsCbyEc" title="YouTube video player" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen></iframe>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-12">
            ...
            <div>
              ...
              <div className="space-y-3 text-gray-400 text-sm">
                <p>Email: support@homnayangi.vn</p><p>Điện thoại: 035 7188616</p><p>Địa chỉ: Trường Đại học Ngoại ngữ - Đại học Quốc gia Hà Nội</p>
              </div>
            </div>
            <div>
