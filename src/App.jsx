    ...
    <div className="min-h-screen flex flex-col font-sans bg-bgLight selection:bg-brand selection:text-white">
      ...
      <main id="hero" className="container mx-auto px-6 py-10 md:py-20 flex flex-col md:flex-row items-center gap-12 lg:gap-20">
        ...
        <div className="w-full md:w-1/2 relative h-[450px] md:h-[600px] rounded-[32px] overflow-hidden shadow-2xl">
          ...
          <button onClick={handleRandomize} className={`absolute bottom-8 right-8 ${randomFood ? 'bg-white text-brand' : 'bg-yellow-400/90 text-yellow-900'} backdrop-blur-md font-bold px-6 py-4 rounded-full flex items-center gap-3 shadow-2xl hover:scale-105 transition-all outline-none border-4 border-white/20`}>
            <span className={`text-2xl ${isSpinning ? 'animate-spin' : ''}`}>🎲</span>
            <span className="text-base md:text-lg">{randomFood || "Ngẫu nhiên thông minh!"}</span>
          </button>
        </div>
      </main>
